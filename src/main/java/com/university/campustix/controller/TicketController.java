...
import com.university.campustix.service.KafkaProducerService;
...
public class TicketController {
    ...
    public ResponseEntity<String> claimTicket(
            ...
            @RequestParam String studentName) { // Add this

        if (!rateLimiter.isAllowed(studentId)) {
            return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body("Rate limit hit.");
        }
        // Send email, seat, and name as a 3-part message
        kafkaProducer.sendBookingRequest(studentId + ":" + seatId + ":" + studentName);
